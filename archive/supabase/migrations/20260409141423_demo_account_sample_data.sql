...
BEGIN
  ...
  INTO v_demo_user_id
  FROM auth.users
  WHERE email = 'demo@gnambaservices.ci'
  LIMIT 1;

  IF v_demo_user_id IS NULL THEN
    RAISE NOTICE 'Demo auth user not found in auth.users, skipping user_profiles seed';
  ELSE
...
VALUES
  ...
  ('contact_address', 'Cocody Angré, Abidjan, Côte d''Ivoire'),
  ('contact_phone', '+225 07 09 00 00 00'),
  ('contact_email', 'contact@gnambaservices.ci'),
  ('contact_hours', 'Lun-Ven : 08h00 - 18h00'),
  ('seo_description', 'Gnamba Services accompagne les projets BTP, immobiliers et fonciers en Côte d''Ivoire.'),
  ('seo_keywords', 'BTP, immobilier, foncier, Abidjan, Gnamba Services'),
  ('logo_url', '')
...
VALUES
  ('10000000-0000-4000-8000-000000000001', 'Villa R+2 Cocody Angré', 'Construction d''une villa haut standing avec piscine, jardin et finitions premium.', 'btp', 2025, 'Cocody, Abidjan', true, 1),
  ('10000000-0000-4000-8000-000000000002', 'Rénovation SGBCI Plateau', 'Rénovation complète de bureaux professionnels sur 1 200 m² avec phasage sans interruption.', 'btp', 2026, 'Plateau, Abidjan', true, 2),
  ('10000000-0000-4000-8000-000000000003', 'Résidence Les Palmiers', 'Gestion locative et suivi d''encaissement pour une résidence de 45 appartements.', 'immobilier', 2025, 'Riviera 3, Abidjan', true, 3),
  ('10000000-0000-4000-8000-000000000004', 'Lotissement Gnamba Village', 'Viabilisation et commercialisation maîtrisée d''un lotissement de 80 parcelles.', 'foncier', 2025, 'Songon, Abidjan', true, 4)
ON CONFLICT (id) DO UPDATE SET
