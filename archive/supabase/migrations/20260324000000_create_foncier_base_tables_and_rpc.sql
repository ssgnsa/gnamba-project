...
-- Version temporaire - la migration 20260408030000 créera la version définitive
-- ============================================
...
DECLARE
  v_role TEXT;
BEGIN
  -- Essayer de récupérer le role depuis user_profiles (créée dans la migration suivante)
  BEGIN
  ...
  -- Fallback sur les metadata de auth.users
  IF v_role IS NULL THEN
    SELECT COALESCE(raw_user_meta_data->>'role', 'employe') INTO v_role
    FROM auth.users WHERE id = auth.uid();
  END IF;
  
  RETURN COALESCE(v_role, 'employe');
END;
...
CREATE INDEX IF NOT EXISTS idx_foncier_lots_deleted_at
  ON foncier_lots(deleted_at) WHERE deleted_at IS NOT NULL;

-- ============================================
-- NOTE: La clé étrangère foncier_attestations.lot_id → foncier_lots.id
-- sera créée dans la migration 20260326000000_create_foncier_attestations_tables.sql
-- car foncier_attestations n'existe pas encore à ce stade.
-- ============================================
...
-- ============================================
-- 11. TRIGGER: updated_at for foncier_lots
-- ============================================
