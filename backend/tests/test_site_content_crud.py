...
def test_site_content_crud_flow() -> None:
    ...
    resp = client.post("/api/v1/site-content", json=payload)
    assert resp.status_code == 200, resp.text
    assert resp.json().get("status") == "ok"

    # Read list and find our key
    list_resp = client.get("/api/v1/site-content")
    ...
    tbl = client.get("/api/v1/tables/site_content")
    ...
    if maybe_id:
        patch_resp = client.patch(f"/api/v1/site-content/{maybe_id}", json={"section": "hero", "key": "test_title", "value": "Titre modifié"})
        assert patch_resp.status_code == 200
        assert patch_resp.json().get("status") == "ok"

        del_resp = client.delete(f"/api/v1/site-content/{maybe_id}")
