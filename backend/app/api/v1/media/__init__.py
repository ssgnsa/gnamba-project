...
from backend.app.api.deps import get_media_service, get_optional_current_user
from backend.app.services.media_service import MediaService

router = APIRouter(prefix="/api/media", tags=["media"])
v1_media_router = APIRouter(prefix="/api/v1/media", tags=["media"])
...
@router.get("/brand-assets", response_model=list[BrandAssetResponse])
def list_brand_assets() -> list[BrandAssetResponse]:
    try:
        service = get_media_service()
        rows = service.list_media(include_deleted=False)
        return [BrandAssetResponse(brand_asset_type=row.get("brand_asset_type"), url=row.get("url") or "") for row in rows if row.get("is_brand_asset")]
    except Exception:
        return []
...
v1_media_router.add_api_route(
    ...
    list_media_usage,
    methods=["GET"],
    response_model=list[MediaResponse | MediaUsageResponse],
)
v1_media_router.add_api_route(
    "/usage",
    create_media_usage,
...
v1_media_router.add_api_route(
    ...
    purge_media,
    methods=["DELETE"],
    response_model=MediaPurgeResponse,
)
v1_media_router.add_api_route(
    "/{media_id}/replace",
    replace_media,
