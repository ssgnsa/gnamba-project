...
@router.get("/properties", response_model=list[ImmobilierResponse])
def list_properties(db: Session = Depends(get_db)) -> list[ImmobilierResponse]:
    return [ImmobilierResponse(**item) for item in _repository(db).list(order_by="created_at")]


@router.post("/properties", response_model=ImmobilierResponse)
def create_property(payload: ImmobilierCreateRequest, db: Session = Depends(get_db)) -> ImmobilierResponse:
    item = _repository(db).create(payload.model_dump(exclude_unset=True))
    return ImmobilierResponse(**item)