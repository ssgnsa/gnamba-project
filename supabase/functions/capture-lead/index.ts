...
export const handler = async (req: Request): Promise<Response> => {
  ...
  if (!checkRateLimit(ip)) {
    return jsonResponse({ error: 'Trop de requêtes. Réessayez dans 1 minute.' }, 429, req)
  }
  ...
  try {
    ...
    const safeSourcePage = source_page ? String(source_page).slice(0, 255) : null
    const safeSourceForm = source_form ? String(source_form).slice(0, 100) : null
    const safeConsent = consent_text ? String(consent_text).slice(0, 500) : 'Consentement via API'
