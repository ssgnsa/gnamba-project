...
usage() {
  cat <<'EOF'
Refresh EGS schema snapshot

Usage:
  scripts/refresh-egs-schema.sh
  scripts/refresh-egs-schema.sh --from-file /tmp/egs-schema-types.ts
  scripts/refresh-egs-schema.sh --project-ref thykrnoqgylrbfupophs
...
if [ ! -s "$tmp_types" ]; then
  if [ -s /tmp/egs-schema-types.ts ]; then
    cp /tmp/egs-schema-types.ts "$tmp_types"
    echo "warning: using fallback snapshot from /tmp/egs-schema-types.ts" >&2
  elif [ -s "$GENERATED_DIR/egs-schema.types.ts" ]; then
    cp "$GENERATED_DIR/egs-schema.types.ts" "$tmp_types"
    echo "warning: using existing generated snapshot from $GENERATED_DIR/egs-schema.types.ts" >&2
  else
...
mv "$tmp_types" "$GENERATED_DIR/egs-schema.types.ts"
mv "$tmp_manifest" "$GENERATED_DIR/egs-schema.json"
mv "$tmp_objects" "$GENERATED_DIR/egs-schema-objects.txt"

echo "types_file=$GENERATED_DIR/egs-schema.types.ts"
echo "manifest_file=$GENERATED_DIR/egs-schema.json"
echo "objects_file=$GENERATED_DIR/egs-schema-objects.txt"
