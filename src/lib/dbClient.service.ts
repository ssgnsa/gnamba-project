/**
 * DataService - Service unifié pour les requêtes API locale
 *
 * Fonctionnalités:
 ...
 */
const withBackoff = async <T extends { data?: any; error?: any }>(
  fn: () => PromiseLike<T>,
  retries = 3,
  baseMs = 500,
): Promise<T> => {
...
/**
 * Classe de service pour les opérations API locale
 */
export class DataService {
  private client: any; // Proxy adapter compatible with API locale client
  ...
  constructor(client: any = dbClient) {
    this.client = client;
  }
  ...
  /**
   * Exécute une requête API locale avec retry automatique
   */
  async queryWithRetry<T>(
    ...
    try {
      ...
      if (import.meta.env.DEV)
        console.error("DataService: queryWithRetry error", error);
      return {
        data: null,
  ...
  async getVillages(): Promise<{
    ...
    if (result.data) {
      const villageNames = (result.data as { nom: string }[]).map((row) => row.nom);
  ...
  /**
   * Signe une attestation via l'API métier versionnée.
   */
  async signAttestation(
    ...
    try {
      const result = await apiClient.foncier.signAttestation(
        attestationId,
        payload,
      );
      if (result.error) {
